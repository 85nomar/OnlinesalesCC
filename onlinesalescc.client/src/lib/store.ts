...
interface StoreState {
  ...
  addAlternativeItem: (orderArtikelNr: number, item: AlternativeItem) => void;
  removeAlternativeItem: (orderArtikelNr: number, alternativeArtikelNr: number) => void;
}
    ...
    (set) => ({
      ...
      updateTicket: (id, updatedTicket) => set((state) => ({
        tickets: state.tickets.map(ticket =>
          ticket.ticketId === id ? { ...ticket, ...updatedTicket } : ticket
        )
      })),
          ...
          updatedAdditionalData = state.ordersGroupedAdditional.map(entry => {
            if (entry.ArtikelNr === artikelNr) {
              ...
              const isAlreadyAdded = currentAlternatives.some(
                (alt: AlternativeItem) => alt.ArtikelNr === item.ArtikelNr
              );
          ...
          if (entry.ArtikelNr === artikelNr && entry.alternativeItems) {
            return {
              ...
              alternativeItems: entry.alternativeItems.filter(
                (item: AlternativeItem) => item.ArtikelNr !== altArtikelNr
              )
            };
          }
          return entry;
